...
import { useContainer } from "../../../../../lib/hooks/useContainers";
...
export default function EditPurchaseOrderPage() {
  ...
  const { container, loading: hookLoading } = useContainer(containerId);
  ...
  // Update form data when container is loaded
  useEffect(() => {
    if (container) {
      setFormData({
        po_id: container.po_id || "",
        container_code: container.container_code || "",
        container_number: container.container_number || "",
        supplier_id: container.supplier_id || "",
        arrival_date: container.arrival_date || "",
        arrival_branch_id: container.arrival_branch_id || "",
        total_container_revenue: container.total_container_revenue || 0,
        items_in_stock: container.items_in_stock || 0,
  ...
  useEffect(() => {
    setContainerLoading(hookLoading);
  }, [hookLoading]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await containerService.update(containerId, payload);
    ...
    return (
      <div className="space-y-8 pb-12 w-full max-w-full overflow-hidden">
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-gray-500">Loading purchase order data...</div>
        </div>
      </div>
    );
  }
