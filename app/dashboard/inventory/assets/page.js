...
export default function AssetsPage() {
  ...
  useEffect(() => {
    const hasActive = searchQuery !== "" || statusFilter !== "All";
    if (hasActive) {
  ...
  const handleOpenDocuments = async (asset) => {
    setSelectedAsset(asset);
    setDocumentsModalOpen(true);
    await fetchDocuments(asset.id);
  };

  const fetchDocuments = async (assetId) => {
    setLoadingDocuments(true);
    try {
      const docs = await assetService.getDocuments(assetId);
  ...
  const handleFileUpload = async (e, documentName) => {
    ...
    try {
      await assetService.uploadDocument(selectedAsset.id, file, documentName);
      success("Document uploaded successfully!");
      await fetchDocuments(selectedAsset.id);
    } catch (err) {
  ...
  const handleDeleteDocument = async (documentId) => {
    if (!selectedAsset) return;
    try {
      await assetService.deleteDocument(documentId);
  ...
  const handleDownloadDocument = async (documentId) => {
    if (!selectedAsset) return;
    try {
      await assetService.downloadDocument(selectedAsset.id, documentId);
    } catch (err) {
  ...
  const getStatusBadge = (status) => {
    ...
    const styles = {
      ...
      disposed: 'bg-gray-100 text-gray-600 dark:bg-zinc-800 dark:text-gray-400',
      maintenance: 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/20 dark:text-yellow-400'
    };
    return (
      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border border-current opacity-80 ${styles[s] || styles.active}`}>
        {s}
      </span>
    );
  };
