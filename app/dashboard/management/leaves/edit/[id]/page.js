...
export default function EditLeavePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (leaveData) {
        setFormData({
          employee_id: leaveData.employee_id?.toString() || "",
          leave_type: leaveData.leave_type || "annual",
          start_date: leaveData.start_date || "",
          end_date: leaveData.end_date || "",
          total_days: leaveData.total_days || 1,
          reason: leaveData.reason || "",
          proof_documents: leaveData.proof_documents || [],
        });
      }
    } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-gray-500">Loading leave data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <ProtectedRoute permission={PERMISSIONS.LEAVES.UPDATE}>
      <div className="space-y-8 pb-12">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Leave Request</h1>
            <p className="text-gray-500 text-sm">Update leave request details</p>
          </div>
        </div>
