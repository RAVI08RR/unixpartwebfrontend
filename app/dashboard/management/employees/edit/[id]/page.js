...
export default function EditEmployeePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const fetchedStatus = employee.insurance_status || "";
      let calculatedStatus = fetchedStatus;
      if (employee.is_insurance_under_process) {
        calculatedStatus = "under_process";
      } else if (!calculatedStatus) {
        const expiryDate = employee.insurance_expiry || "";
        if (!expiryDate) {
      ...
      setFormData({
        ...
        insurance_agent_name: employee.insurance_agent_name || "",
        insurance_status: calculatedStatus,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        branch_on_visa_id: formData.branch_on_visa_id ? parseInt(formData.branch_on_visa_id) : 0,
  ...
  return (
    <ProtectedRoute permission={PERMISSIONS.EMPLOYEES.UPDATE}>
