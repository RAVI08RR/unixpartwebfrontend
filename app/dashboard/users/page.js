...
import {
  ...
  Filter, Plus,
  ShieldCheck, Pencil, Trash2, Check, X, Eye, User, Building2, Store, Calendar, RotateCcw
} from "lucide-react";
...
export default function UserManagementPage() {
  ...
  const PAGE_SIZE = 10;
  const { users: apiUsers, isLoading, isError, mutate, total, totalPages } = useUsers(currentPage, PAGE_SIZE);
  ...
  // Auto-expand filters if statusFilter is not default on load
  useEffect(() => {
    if (statusFilter !== "All") {
  ...
  // Show all items from current backend page (no client-side slicing)
  ...
  const filteredUsers = paginatedUsers; // alias for export
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-[24px] border border-gray-100 dark:border-zinc-800 shadow-sm p-6 space-y-4 animate-in fade-in slide-in-from-top-1 duration-200">
            <div className="flex justify-between items-center pb-2 border-b border-gray-50 dark:border-zinc-800/50">
              <div>
                ...
                <p className="text-xs text-gray-400 dark:text-zinc-500 font-medium">Refine the user list below.</p>
              </div>
              {statusFilter !== "All" && (
                <button
                  onClick={() => {
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-gray-50 dark:bg-zinc-800/50 rounded-xl p-4">
                      ...
                      <div className={`inline-flex mt-3 role-badge ${selectedUser.role?.name?.toLowerCase() === 'administrator' ? 'role-badge-admin' :
