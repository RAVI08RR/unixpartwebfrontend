...
import { ArrowLeft, Truck, Save, X, Percent, Loader2 } from "lucide-react";
...
export default function EditBranchOwnerPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const ownerData = await branchOwnerService.getById(params.id);
      console.log('Branch owner data loaded:', ownerData);
      
      setFormData({
        branch_id: ownerData.branch_id || "",
        supplier_id: ownerData.supplier_id || "",
        share_percent: ownerData.share_percent || "",
        share_amount: ownerData.share_amount || "0",
      });
    } catch (err) {
      console.error('Failed to fetch data:', err);
      showError('Failed to load branch owner data');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Updating branch owner with data:", submitData);
      
      await branchOwnerService.update(params.id, submitData);
      
      // Success - redirect to branch owners list
      success("Branch owner updated successfully!");
      router.push("/dashboard/administration/branch-owners");
      
    } catch (err) {
      console.error("Failed to update branch owner:", err);
      setError(err.message || "Failed to update branch owner. Please try again.");
      showError(err.message || "Failed to update branch owner");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-gray-500">Loading branch owner data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-12 w-full max-w-full overflow-hidden">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black dark:text-white tracking-tight">Edit Branch Owner</h1>
          <p className="text-gray-400 dark:text-white text-sm font-normal">Update branch ownership information</p>
        </div>
      </div>
        ...
        <div className="p-8">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">Update the ownership details below</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center gap-4 pt-6 border-t border-gray-100 dark:border-zinc-800">
              ...
              >
                ...
                <span>{isLoading ? "Updating..." : "Update Branch Owner"}</span>
              </button>
